...
TOKEN = os.environ.get("BOT_TOKEN", "8300157614:AAHL0CKYHxl4dxzgt-rIU5bicsVeB6ndIT0")
...
MEDICAL_ITEMS = {
    "adrenaline": {"name": "💉 أدرينالين", "desc": "إنقاذ ذاتي تلقائي"},
    "scalpel": {"name": "🗡️ مشرط صدئ", "desc": "طعن نهاراً"},
    "detector": {"name": "🔍 كاشف كذب", "desc": "كشف هوية فورية"},
}
...
def get_rank(uid):
    ...
    if xp >= 10000: return "👑 ملك المستشفى"
    elif xp >= 5000: return "💎 أسطوري"
    elif xp >= 2000: return "🥇 خبير"
    elif xp >= 500: return "🥈 متمرّس"
    else: return "🥉 مبتدئ"
...
def group_cmd(m):
    ...
    deletable = {"/hospital", "/vote", "/force_start", "/cancel", "/done", "/time", "/suspect", "/commands", "/hall", "/rooms_cancel", "/لاسلكي", "/كشف_الكاميرات"}
...
def init_game(msg, gtype):
    ...
    try:
        if gtype == "hospital":
            m2 = bot.send_animation(cid, asset, caption=txt, parse_mode="HTML", reply_markup=mk)
        else:
            m2 = bot.send_photo(cid, asset, caption=txt, parse_mode="HTML", reply_markup=mk)
    except:
...
def do_commands(m):
    ...
    cmd_text = (
        ...
        "<code>/kill</code> - استخدام المشرط (نهاري)\n"
